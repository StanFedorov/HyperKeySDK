...
static const CGFloat kSidePullViewWidth = 70.0;
...
- (id)initWithScrollView:(UIScrollView *)scroll position:(AllAroundPullViewPosition)position
{
    ...
    if ((self = [super initWithFrame:frame])) {
        ...
        if (self.isSideView) {
            CGFloat sideArrowPositionOffset = (self.position == AllAroundPullViewPositionLeft) ? kSidePullViewWidth - 4.0 : self.frame.size.width - 15.0;
...
- (void)parkVisible
{
    ...
    else if (self.position == AllAroundPullViewPositionLeft)
        self.scrollView.contentInset = UIEdgeInsetsMake(self.originalInset.top, kViewHeight + self.originalInset.left, self.originalInset.bottom, self.originalInset.right);
