...
- (void)setDefaults {
    ...
    _scrollingTriggerEdgeInsets = UIEdgeInsetsMake(8.0f, 8.0f, 8.0f, 8.0f);//UIEdgeInsetsMake(50.0f, 50.0f, 50.0f, 50.0f);
}
...
- (void)invalidateLayoutIfNecessary {
    ...
    [self.collectionView performBatchUpdates:^{
        ...
        if (strongSelf) {
            ...
            //NSLog(@"%li ➡️ %li",(long)previousIndexPath.row,(long)newIndexPath.row);
        }
...
- (void)setupScrollTimerInDirection:(LXScrollingDirection)direction {
    ...
    if ([self.delegate respondsToSelector:@selector(collectionView:willScrollOverLimitWithDirection:)]) {
        [self.delegate collectionView:self.collectionView willScrollOverLimitWithDirection:direction];
    }
...
- (void)handleScroll:(CADisplayLink *)displayLink {
    ...
    if ([self.delegate respondsToSelector:@selector(collectionView:didEndScrollOverLimitWithDirection:)]) {
        [self.delegate collectionView:self.collectionView didEndScrollOverLimitWithDirection:direction];
    }
}
...
- (void)handleTapGesture:(UITapGestureRecognizer*)gestureRecognizer{
    ...
    if ([self.delegate respondsToSelector:@selector(collectionView:layout:didTapDraggingItemAtIndexPath:)]) {
        [self.delegate collectionView:self.collectionView layout:self didTapDraggingItemAtIndexPath:indexPath];
    }
